...
В расширении \code{server_name} клиент передает имя сервера, с которым он 
...
Расширение \code{max_fragment_length} используется для согласования
максимальной длины фрагментов данных при их передаче по протоколу Record.

\item\label{HS.Ext.alpn}
Расширение \code{application_layer_protocol_negotiation} используется  
для согласования прикладного протокола, который будет выполняться поверх TLS.

\item\label{HS.Ext.ct}
Расширения \code{client_certificate_type} и \code{server_certificate_type} 
...
\item\label{HS.Ext.p}
Расширение \code{padding} используется для искусственного увеличения размера 
...
\subsection{Расширение \code{supported_versions}}\label{HS.Ext.sv} 

Клиент использует расширение \code{supported_versions} для информирования о 
поддерживаемых версиях TLS, сервер~--- для информирования о выбранной версии. 

Данные расширения \code{supported_versions} описываются типом 
...
Клиент должен включить расширение \code{supported_versions} в сообщение 
...
\code{supported_versions} и если сервер поддерживает TLS версии 1.2, то 
...
\code{supported_versions}, то он должен игнорировать
...
расширении~\code{supported_versions} которого отсутствует версия \code{0x0304}. 

Если сервер выбрал версию 1.2, то он должен указать эту версию в поле 
\code{ServerHello.version} и не должен включать в \token[HS.SH]{ServerHello} 
расширение \code{supported_versions}.
%
...
Клиент должен проверять наличие \code{supported_versions} в 
...
\subsection{Расширение \code{cookie}}\label{HS.Ext.c}
...
Данные расширения \code{cookie} описываются типом \code{Cookie}, который 
определяется следующим образом:
...
Сервер может отправить расширение \code{cookie} в сообщении
\token[HS.HRR]{HelloRetryRequest}. 
%
После получения \token[HS.HRR]{HelloRetryRequest} с расширением \code{cookie} 
...
\token[HS.CH]{ClientHello} в последующих соединениях с сервером.

\begin{note}
Отправка \code{cookie} в \token[HS.HRR]{HelloRetryRequest} является исключением 
...
Сервер может указать в \code{cookie} хэш-значение первого сообщения
\token[HS.CH]{ClientHello}. Получив повторное \token[HS.CH]{ClientHello},
сервер реконструирует первое, вычисляет его хэш-значение и сравнивает с 
хэш-значением, указанным в \code{cookie}. При такой схеме взаимодействия 
серверу не нужно поддерживать состояние соединения с клиентом и хранить в нем 
первое \token[HS.CH]{ClientHello}.
...
первое сообщение \token[HS.CH]{ClientHello} от повторного. Между этими 
сообщениями сервер может получить незащищенную запись типа 
\code{change_cipher_spec}. Сервер без состояния должен игнорировать эту запись 
(см.~\ref{RECORD.Rec}).
\end{note}

\subsection{Расширения \code{signature_algorithms} и 
\code{signature_algorithms_cert}}\label{HS.Ext.sa} 

Клиент и сервер используют расширения \code{signature_algorithms} и 
\code{signature_algorithms_cert} для информирования о поддерживаемых алгоритмах 
ЭЦП. Первое расширение касается подписи в сообщении 
\token[HS.CV]{CertificateVerify}, второе~--- подписей сертификатов.

Данные расширений \code{signature_algorithms} и 
\code{signature_algorithms_cert} описываются типом \code{SignatureSchemeList}, который 
определяется следующим образом:
...
\code{signature_algorithms} в сообщение \token[HS.CH]{ClientHello}.
%
...
сообщение должно содержать расширение~\code{signature_algorithms}.

Если сторона аутентифицируется по сертификату, но не получила 
\code{signature_algorithms} в \token[HS.CH]{ClientHello} или 
...
в~\code{signature_algorithms}. 

Расширение~\code{signature_algorithms} может сопровождаться 
расширением~\code{signature_algorithms_cert}. В последнем расширении 
перечисляются алгоритмы, с помощью которых проверяются подписи в цепочке 
сертификатов аутентифицирируемой стороны. Если 
расширение~\code{signature_algorithms_cert} опущено, то перечень алгоритмов   
определяется по~\code{signature_algorithms}. 

Алгоритмы проверки подписей в цепочке сертификатов должны входить в 
перечень~\code{signature_algorithms_cert} или, если это расширение опущено,  
в перечень~\code{signature_algorithms}. 
%
...
\subsection{Расширение \code{certificate_authorities}}\label{HS.Ext.ca} 

Расширение \code{certificate_authorities} используется для информирования
об УЦ, которым отправитель доверяет. Получателю следует учитывать 
информацию при выборе сертификата для аутентификации.

Данные расширения \code{certificate_authorities} описываются типом 
...
Клиент может отправить расширение \code{certificate_authorities}
в сообщении~\token[HS.CH]{ClientHello}, сервер~---
в сообщении~\token[HS.CR]{CertificateRequest}. 

\subsection{Расширение \code{oid_filters}}\label{HS.Ext.of} 

В расширении \code{oid_filters} сервер указывает требования к сертификатам, 
...
Данные расширения \code{oid_filters} описываются типом 
...
Если сервер использует расширение~\code{oid_filters}, то он должен 
включить его в сообщение~\token[HS.CR]{CertificateRequest}. 
%
Идентификаторы расширений сертификата в~\code{oid_filters} не должны 
повторяться.

Если сервер включил~\code{oid_filters} в~\token[HS.CR]{CertificateRequest}, то 
ответный сертификат клиента должен содержать все расширения, перечисленные
в~\code{oid_filters} и распознаваемые клиентом.
%
В этих расширениях должны присутствовать все требуемые значения.
%
Клиент должен игнорировать перечисленные в~\code{oid_filters} расширения 
...
сертификат, который не полностью соответствует требованиям~\code{oid_filters}, 
...
требованиям~\code{oid_filters}, если каждый бит назначения ключа, установленный 
в~\code{oid_filters}, установлен также в сертификате.
\item
Расширение \code{ExtKeyUsage} (название в соответствии с СТБ 34.101.78) в 
сертификате клиента соответствует требованиям~\code{oid_filters}, если каждый 
идентификатор области применения, указанный в~\code{oid_filters}, указан также 
в сертификате.
%
В~\code{oid_filters} не должен указываться идентификатор 
...
\subsection{Расширение \code{status_request}}\label{HS.Ext.sr} 

Расширение \code{status_request} используется для проверки статуса сертификата, 
используемого для аутентификации. Статус проверяется с помощью протокола 
OCSP, установленного в СТБ 34.101.26.

Данные расширения \code{status_request} описываются типом 
...
\code{status_request} в сообщение \token[HS.CR]{CertificateRequest}.
%
Получив запрос, клиент в сообщении~\token[HS.CT]{Certificate} вместе со 
своим сертификатом может выслать расширение \code{status_request} с 
OCSP-ответом о статусе сертификата.

\subsection{Расширение \code{post_handshake_auth}}\label{HS.Ext.pha} 

Расширение \code{post_handshake_auth} используется для информирования о том,
что клиент готов выполнить аутентификацию после Handshake 
(см.~\ref{HS.Post.Auth}). 

Расширение \code{post_handshake_auth} не содержит данные. Формально данные 
...
Если клиент использует расширение \code{post_handshake_auth}, то он должен 
включить его в сообщение \token[HS.CH]{ClientHello}.
%
...
Сервер не должен отправлять расширение \code{post_handshake_auth}.

\subsection{Расширение \code{supported_groups}}\label{HS.Ext.sg} 

В расширении \code{supported_groups} отправитель перечисляет циклические 
группы, которые он может использовать в протоколе Диффи~-- Хеллмана.

Данные расширения \code{supported_groups} описываются типом 
...
Клиент включает расширение \code{supported_groups} в сообщение 
...
Сервер может включить расширение \code{supported_groups} в сообщение
...
расширение \code{supported_groups}, проинформировав тем самым клиента о своих 
предпочтениях. При этом серверу следует указать в \code{supported_groups} все 
поддерживаемые группы, независимо от того, поддерживаются они клиентом или нет. 

\subsection{Расширение \code{key_share}}\label{HS.Ext.ks} 

Расширение \code{key_share} содержит данные обмена между сторонами
...
Клиент отправляет расширение \code{key_share} в сообщении 
\token[HS.CH]{ClientHello} вместе с расширением 
\token[HS.Ext.sg]{supported_groups} (см.~\ref{HS.CH}).

Данные расширения \code{key_share} в \token[HS.CH]{ClientHello} описываются 
...
следует сначала выбрать группу на основании \code{supported_groups}, а затем,
...
группы, указанной в \code{supported_groups}. Открытые ключи должны 
...
групп, не указанных в \code{supported_groups}.
%
...
\code{HelloRetryRequest} с включением в него расширения~\code{key_share}.

Данные расширения \code{key_share} в \token[HS.HRR]{HelloRetryRequest} 
...
Получив расширение \code{key_share} в \token[HS.HRR]{HelloRetryRequest}, 
клиент должен проверить два условия:
\begin{enumerate}[label=\arabic*)]
\item
в \code{selected_group} указана одна из групп списка \code{supported_groups};
\item
в \code{selected_group} не указана группа, для которой в расширении 
\code{key_share} предыдущего сообщения \token[HS.CH]{ClientHello}
...
\code{key_share}. В новую редакцию расширения должен быть включен только один 
...
Если расширение \code{key_share} клиента устраивает сервер и он может 
продолжить Handshake без перезапуска, то сервер отвечает сообщением 
\token[HS.SH]{ServeHello}, также включая в него \code{key_share}.

Данные расширения \code{key_share} в \token[HS.SH]{ServerHello} 
...
представил открытый ключ в своем расширении \code{key_share}.
%
\doubt{Сервер не должен отправлять открытый ключ для групп, не включенных 
клиентом в расширение \code{supported_groups}.}
%
...
\subsection{Расширение \code{psk_key_exchange_modes}}\label{HS.Ext.pkem} 

В расширении \code{psk_key_exchange_modes} клиент сообщает, какие режимы 
...
Данные расширения \code{psk_key_exchange_modes} описываются типом 
...
расширением \code{psk_key_exchange_modes}.
%
Если сервер получает \token[HS.CH]{ClientHello} с 
\token[HS.Ext.psk]{pre_shared_key}, но без \code{psk_key_exchange_modes}, 
...
Сервер не должен отправлять расширение \code{psk_key_exchange_modes}.

\subsection{Расширение \code{early_data}}\label{HS.Ext.ed} 

Расширение \code{early_data} используется для информирования о применении 
...
\token[HS.Ext.psk]{pre_shared_key} и \code{early_data}. В первом расширении 
...
содержать расширение \code{early_data}. В поле 
...
\code{early_data}, должен поступить одним из следующих способов.

\begin{enumerate}
\item
Сервер игнорирует расширение \code{early_data} и действует по правилам 1-RTT,
...
%
Клиент не должен включать \code{early_data} в повторное сообщение 
...
\token[HS.EE]{EncryptedExtensions} с расширением \code{early_data}.
%
...
\code{ticket_age_add} сообщения~\token{NewSessionTicket} (см.~\ref{HS.NST}).
%
...
Если сервер принял расширение \code{early_data}, то он должен обрабатывать 
...
с расширением \code{early_data}, не дожидаясь сообщения 
\token[HS.EOED]{EndOfEarlyData} от клиента.

Если сервер отклоняет \code{early_data}, то клиент может повторить отправку 
...
\subsection{Расширение \code{pre_shared_key}}\label{HS.Ext.psk} 

Расширение \code{pre_shared_key} используется для согласования 
...
Данные расширения \code{pre_shared_key} описываются типом 
...
Клиент отправляет расширение \code{pre_shared_key} в сообщении 
...
Сервер отправляет расширение \code{pre_shared_key} в сообщении 
...
\code{pre_shared_key} и проверяет, что указанный в нем номер 
...
Чтобы упростить построение PSK-скрепок, расширение \code{pre_shared_key} должно 
