...
%
Расширения, помеченные звездочкой, должны быть реализованы обязательно, если 
противное не оговаривается за пределами настоящего стандарта относительно 
конкретного случая применения TLS.
...
используются для согласования типа сертификата для аутентификации клиента и 
сервера соответственно. Поддерживаются два типа сертификатов: обычные 
сертификаты X.509, определенные в СТБ 34.101.19, и контейнеры типа 
...
\hline
\hline
\code|server_name|$\phantom{}^*$ & \cite{RFC6066} & 
CH, EE\\ 
%
...
% todo: \cite{RFC6066}?
%
\code|supported_groups|$\phantom{}^*$ & \ref{HS.Ext.sg} & 
CH, EE\\ 
% todo: \cite{RFC7919}?
%
\code|signature_algorithms|$\phantom{}^*$ & \ref{HS.Ext.sa} &
CH, CR\\ 
%
...
CH, EE, NST\\ 
%
\code|supported_versions|$\phantom{}^*$ & \ref{HS.Ext.sv} &
CH, SH, HRR\\ 
%
\code|cookie|$\phantom{}^*$ & \ref{HS.Ext.c} &
CH, HRR\\ 
%
...
CH\\ 
%
\code|signature_algorithms_cert|$\phantom{}^*$ & \ref{HS.Ext.sa} &
CH, CR\\ 
%
\code|key_share|$\phantom{}^*$ & \ref{HS.Ext.ks} &
CH, SH, HRR\\ 
\hline
\end{tabular}
\end{table}
...
проектировать так, чтобы противник не мог навязать использование (или, 
наоборот, отключение) определенных функциональных возможностей TLS. 
\end{itemize}
...
\code{ServerHello.legacy_version} (\code{ServerHello.version} в TLS версии 1.2) 
...
должна быть указана выбранная версия (\code{0x0304}). Кроме этого, сервер 
...
соответствовать одному из алгоритмов, указанных в~\token{signature_algorithms}.
...
Расширение \token{post_handshake_auth} не содержит данных. Формально данные 
...
группы. Форматы открытых ключей для некоторых известных групп определены
в~\cite{RFC8446}, а также в~\ref{BSUITES.DH}.
\end{itemize}
...
\token[HS.Ext.sg]{supported_groups}, и идти в той же очередности.
%
...
\code{OfferedPsks.identities}, предложенном клиентом. Нумерация в списке  
начинается с~$0$.
