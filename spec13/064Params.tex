...
\subsection{Сообщение \code{EncryptedExtensions}}\label{HS.EE} 

В сообщении \code{EncryptedExtensions} сервер пересылает защищенные расширения.
...
Сервер должен выслать \code{EncryptedExtensions} сразу после своего сообщения 
\token[HS.F]{Finished}. 

Сообщение \code{EncryptedExtensions} защищается на ключе, построенном по 
\code{server_handshake_traffic_secret} (см.~\ref{CRYPTO.Schedule}). Это первое 
сообщение, которое защищается на данном ключе.

Клиент должен проверить, что \code{EncryptedExtensions} содержит только 
...
\subsection{Сообщение \code{CertificateRequest}}\label{HS.CR} 

С помощью сообщения \code{CertificateRequest} сервер запрашивает сертификат 
клиента для его аутентификации. Сервер может выслать \code{CertificateRequest}, 
только если сам аутентифицируется по сертификату.

Формат сообщения \code{CertificateRequest} описывается одноименным типом:
%
\begin{codeblock}
struct {
...
%
Если в соединении высылается несколько сообщений \code{CertificateRequest},
то их строки \code{certificate_request_context} не должны повторяться.
%
Если \code{CertificateRequest} высылается в Handshake, а не по его завершении,
то строка должна иметь нулевую длину.
%
При отправке \code{CertificateRequest} после Handshake серверу следует выбирать 
...
Сообщение \code{CertificateRequest} не является обязательным. Если сервер 
высылает \code{CertificateRequest}, то он должен это сделать сразу после 
...
соединении, то он не должен отправлять \code{CertificateRequest} в протоколе 
Handshake, хотя может это сделать по завершении протокола
(см.~\ref{HS.Post.Auth}) при условии, что клиент выслал в
\token[HS.CH]{ClientHello} расширение \token{post_handshake_auth}
(см.~\ref{HS.Ext.pha}).
%
Если сервер аутентифицируется с помощью PSK, согласованного за пределами TLS, 
то он не должен отправлять \code{CertificateRequest} ни в протоколе Handshake, 
...
Сообщение \code{CertificateRequest} защищается на ключе, построенном по 
