...
\subsection{Сообщение \code{Certificate}}\label{HS.CT} 

В сообщении \code{Certificate} сторона передает свой сертификат для
аутентификации, а также другие сертификаты и данные, которые подтверждают его
подлинность.

Формат сообщения \code{Certificate} описывается одноименным типом:
%
\begin{codeblock}
enum {
  X509(0),
...
%
Сертификат отправителя сообщения \code{Certificate} должен быть 
...
что присланный в \code{Certificate} контейнер входит в список.
\end{note*}
...
%
Расширения в сообщении \code{Certificate} сервера должны соответствовать 
расширениям в \token[HS.CH]{ClientHello},  
а расширения в \code{Certificate} клиента~--- расширениям в 
...
\token{status_request} (см.~\ref{HS.Ext.sr}). Перечень допустимых расширений 
может дополняться за пределами стандарта.

% skip: and the SignedCertificateTimestamp extension [RFC6962]

Сервер должен отправить сообщение \code{Certificate}, если 
...
Клиент должен отправить сообщение \code{Certificate}, только и только если
...
сертификат, то клиент должен отправить сообщение \code{Certificate} 
...
Сообщение \code{Certificate} защищается на ключе, построенном по 
...
указанных противоположной стороной в расширении \token{signature_algorithms}
(см.~\ref{HS.Ext.sa}).
...
\subsection{Сообщение \code{CertificateVerify}}\label{HS.CV} 

Сообщение \code{CertificateVerify} содержит подпись текущей стенограммы 
Handshake. С помощью \code{CertificateVerify} отправитель доказывает, что он
действительно владеет личным ключом, связанным с его сертификатом. Кроме этого, 
подпись в \code{CertificateVerify} подтверждает целостность и подлинность 
стенограммы. 

Формат сообщения \code{CertificateVerify} описывается одноименным типом:
...
Сервер должен отправлять \code{CertificateVerify} при аутентификации по 
сертификату, после сообщения \token[HS.CT]{Certificate}.
%
Клиент должен отправлять \code{CertificateVerify} при аутентификации по 
сертификату, если сертификат указан в \token[HS.CT]{Certificate} (список 
сертификатов непуст).

Сообщение \code{CertificateVerify} должно отправляться непосредственно после 
\token[HS.CT]{Certificate} и непосредственно перед \token[HS.F]{Finished}.

Идентификатор алгоритмов ЭЦП в сообщении \code{CertificateVerify} сервера 
...
%
Идентификатор алгоритмов ЭЦП в \code{CertificateVerify} клиента 
должен входить в список, указанный сервером в расширении 
\token[HS.Ext.sa]{signature_algorithms} сообщения 
\token[HS.CR]{CertificateRequest}.  
%
Открытый ключ в сертификате отправителя \code{CertificateVerify} должен 
соответствовать алгоритмам ЭЦП. 

Получатель \code{CertificateVerify} должен проверить подпись в поле
...
Сообщение \code{CertificateVerify} защищается на ключе, построенном по 
\code{client_handshake_traffic_key} или \code{server_handshake_traffic_key}, 
в зависимости от отправителя.

\subsection{Сообщение \code{Finished}}\label{HS.F} 

В сообщении \code{Finished} пересылается имитовставка текущей стенограммы 
...
Формат сообщения \code{Finished} определяется одноименным типом:
...
Получатель \code{Finished} должен проверить имитовставку.
В случае ошибки получатель должен прервать Handshake с 
оповещением \token[ALERT.Err.de2]{decrypt_error}. 

После того, как сервер отправил свое сообщение \code{Finished}, получил и 
проверил ответное \token{Finished}, он может начать отправку прикладных данных. 
%
Сервер может отправлять прикладные данные даже раньше, не дожидаясь \code{Finished} 
...
%
Клиент может отправлять прикладные данные, не дожидаясь \code{Finished} 
сервера, в рамках механизма 0-RTT.

Сообщение \code{Finished} защищается на ключе, построенном по  
\code{client_handshake_traffic_key} или \code{server_handshake_traffic_key}, в 
зависимости от отправителя.

После \code{Finished} отправляемые данные должны защищаться на ключе,
